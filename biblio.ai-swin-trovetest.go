...
func main() {
	...
	stg_rows_url, err := env.db.Query("SELECT distinct url as stg_url from stg_swin_trovetest")
	if err != nil {
	...
	for stg_rows_url.Next() {
		...
		if len(item_id) == 0 {
			...
			stmt_all := `SELECT url,header_identifier, date_latest, metadata_identifier, metadata_identifier_file_id from stg_swin_trovetest where url = $1 limit 1`
			stg_rows_all := env.db.QueryRow(stmt_all, stg_url)
			err = stg_rows_all.Scan(&url, &header_identifier, &date_latest, &metadata_identifier,  &metadata_identifier_file_id)
			//fmt.Println(value)
			if err != nil {
				// handle this error
				panic(err)
			...
			statement_metadata_name := `INSERT INTO item_metadata (item_id, metadata_key, metadata_value) VALUES ($1,$2,$3)`
                        _, err = env.db.Exec(statement_metadata_name, inserted_item_id, "name", "swin-trovetest")
			if err != nil {
				fmt.Println(err)
				return
			}
