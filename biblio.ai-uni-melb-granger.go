...
func main() {
	...
	stg_rows_url, err := env.db.Query("SELECT distinct url as stg_url from stg_uni_melb_granger")
//	stg_rows_url, err := env.db.Query("SELECT distinct url as stg_url from stg_slv_primo where metadata_identifier like 'IE20347212'")
	...
	for stg_rows_url.Next() {
		...
		if len(item_id) == 0 {
			...
			stmt_all := `SELECT url,header_identifier, date_latest, metadata_identifier, metadata_identifier_handle_id, metadata_identifier_cms_id, metadata_identifier_accession_id, metadata_identifier_file_id from stg_uni_melb_granger where url = $1 limit 1`
			...
			statement_metadata_name := `INSERT INTO item_metadata (item_id, metadata_key, metadata_value) VALUES ($1,$2,$3)`
                        _, err = env.db.Exec(statement_metadata_name, inserted_item_id, "name", "uni-melb-granger")
			if err != nil {
				fmt.Println(err)
				return
			}
